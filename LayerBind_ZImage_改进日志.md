...
- 当前分支：`layerbind`
- 当前有效基线：
  - `Phase1/Phase2` 的全局上下文已按论文方向对齐
  - `region-local RoPE` 明显改善了主体位置与占比
  - `t1` 诊断已经确认：灰色/彩色色块源头在 `Phase1 / t1 blend`
  - 当前最有效修复是 `core-first + dominant component` 的 `t1 mask` 精炼
  - 当前新增第一版 `counterfactual text-delta` 主信号：
    - `t1 mask` 不再优先依赖 `branch vs global current`
    - 改为优先使用 `branch vs no-region-text counterfactual branch`
- 当前核心结论：
  - `Phase2` 不是主根因，最多只会放大或缓和现象
  - 真正决定成败的是 `Phase1 branch` 与 `t1 blend`
  - 在 `Z-Image unified self-attention` 上，论文 Appendix A.2 的 `branch-global diff -> foreground alpha` 前提并不天然成立
  - 对 Z-Image 来说，更接近真实前景归属的信号是“region text 对 branch token 的净驱动增量”，而不是单纯 `branch-global diff`
- 当前主线目标：
  - 巩固 `t1 blend` 的稳定性
  - 保留主体结构与背景融合
  - 避免碎 mask、色块和主体形变回归
  - 验证 `counterfactual text-delta` 是否能让 mask 更接近真实主体，而不是中心块状 support

## 4. 关键改进

### 4.1 论文对齐主线

- 将 `Phase1/Phase2` 的 region image context 改回完整全局图像上下文，不再使用局部窗口 + anchors 作为主路径。
- 恢复 `Phase2` 的区域 text 更新回路，改成受控阻尼版本，而不是彻底冻结。
- 不再在预处理阶段对 overlap token 做唯一归属，重叠 token 交由后续 layer-wise blend 处理。
- 这一步解决的是“region 像孤岛”“主体只在局部角落成形”的大方向问题。

### 4.2 几何先验主线

- 为 `Phase1 branch` 和 `Phase2 local path` 引入 `region-local RoPE`。
- 作用是把“主体应该占用这个框”的几何信息显式注入局部路径。
- 这一步是第一批真正带来显著实测收益的改动：
  - 主体占比扩大
  - 位置明显更准

### 4.3 `t1 blend` 主线

- 新增 `t1` 诊断输出，明确确认了色块源头在 `Phase1 / t1 blend`，而不是后续 `Phase2`。
- 在 `t1 blend` 中引入 `core-first blend`：
  - 先用 bbox 几何中心生成稳定 core seed
  - 再从 diff-mask 中只保留与核心最相关的主前景区域
- 进一步将 `core-first` 从简单形态学重建升级为 `dominant connected component selection`：
  - 只保留与 bbox 核心重叠最强、且 alpha 质量最高的主连通域
- 这是当前解决“灰色/彩色色块”“mask 很碎”“结构变形”的最关键改动。

## 5. 局部优化

- `Phase2 delta alpha merge`
  - 改善了融合自然度，但不是最终根因修复。
- `foreground-query-only local attention`
  - 帮助确认 `Phase2` query 范围不是主根因，但主体会变小。
- `bf16/fp16 dtype` 修复
  - 保证低精度推理稳定。
- `t1 debug masks`
  - 为后续排查提供了直接证据，属于高价值诊断能力。

## 6. 无效或已回退尝试

- `Phase2 global path -> background-dominant`
  - 0 收益，且会打掉主体。
- `Phase2 residual suppression`
  - 0 收益，只是后处理残差大小，没碰到根因。
- 过于激进的 `Phase2` 前景 query 限制
  - 色块没明显减少，但主体会变小。
- 仅靠 `diff-alpha` 直接主导 `t1 foreground mask`
  - 在 `Z-Image` 上会产生碎 mask，并把脏块整块写回。
...
- 背景问题：
  - 经过 `core-first + dominant component` 后，`t1` 色块问题已大幅缓解，但导出的 mask 更像接近 bbox 中心的大块轮廓，而不是自然主体形状。
  - 这说明当前 mask 的稳定性问题已经解决，但主信号仍不理想：
    - 不加核心约束时，`branch-global diff` 会碎裂
    - 加上核心约束后，又会退化成几何 support mask
  - 根因不是 morphology 本身，而是 `branch-global diff` 在 Z-Image 上混入了过多局部漂移、纹理噪声和 branch 轨迹偏移，无法直接代表“region text 驱动出的前景”。
- 改动点：
  - 在 `Phase1` 中为每个 region 新增一条 `counterfactual branch`：
    - 与正常 branch 共享同一个初始噪声 patch
    - 共享相同的全局背景上下文
    - 但不接入 `region text`
  - `t1 mask` 估计时，优先使用：
    - `branch_tokens - counterfactual_branch_tokens`
    - 替代原先优先使用的 `branch_tokens - global_current_tokens`
  - 当前仍保留 `core-first + dominant component` 作为稳定化约束，避免直接回退到碎块 mask。
  - 新增测试：验证 `blend_region_tokens` 会优先使用 `counterfactual_branch_tokens` 作为前景信号参考。
- 预期收益：
  - 让前景分数更接近“region text 造成的净增量”，而不是“所有局部漂移的混合差值”。
  - 在不丢失当前稳定性的前提下，使 mask 从“几何 support 块”向“真实主体归属”靠近。
- 已知风险：
  - `Phase1` 额外维护一条 counterfactual branch，推理开销会上升。
  - 在 hard-binding 层中，“无 region text” 分支目前仍是工程近似，并不是论文定义的一部分。
  - 若 counterfactual branch 与正常 branch 在后期轨迹差距过大，也可能引入新的过强差值。
- 验证方式/结果：
