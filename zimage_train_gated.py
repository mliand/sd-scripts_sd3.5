# Training Z-Image with Gated Attention
...
def train(args: argparse.Namespace):
    ...
    logger.info(f"Loading gated Z-Image transformer with gate_type={args.gate_type}")
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if should_log_gate_stats:
                ...
                if hasattr(unwrapped_transformer, "set_log_gate_stats"):
                    unwrapped_transformer.set_log_gate_stats(True)
            ...
            if len(accelerator.trackers) > 0:
                ...
                if should_log_gate_stats:
                    unwrapped_transformer = accelerator.unwrap_model(transformer)
                    if hasattr(unwrapped_transformer, "get_gate_statistics"):
                        gate_stats = unwrapped_transformer.get_gate_statistics()
...
def setup_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Type of gating for attention: headwise, elementwise, or none. Default: headwise",
    )
    parser.add_argument(
        "--log_gate_stats",
        action="store_true",
