...
def test_phase2_keeps_region_branch_trajectory_and_freezes_text_tokens(monkeypatch):
    transformer = create_tiny_zimage_model()
    device = torch.device("cpu")
    cap_mask = torch.tensor([[True, True, True]], device=device)
    cap_feats = torch.randn(1, 3, 12, device=device)
    ...
    phase2_branch_seed = torch.full((1, 1, transformer.dim), 0.25, device=device)
    frozen_text_tokens = region_tokens.clone()
    region_states = [
        {
            "layer_index": 1,
            "bbox": (0, 0, 16, 16),
            ...
            "branch_patches": None,
            "branch_tokens": phase2_branch_seed.clone(),
            "text_tokens": frozen_text_tokens.clone(),
    ...
    global_region_before = transformer.patchify(latent, 2, 1).index_select(1, region_states[0]["indices"])
    ...
    assert len(captured_queries) == len(transformer.layers)
