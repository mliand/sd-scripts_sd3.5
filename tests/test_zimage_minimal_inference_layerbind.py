...
def test_select_phase2_query_positions_prefers_alpha_foreground():
    region_state = {
        "indices": torch.tensor([10, 11, 12, 13], dtype=torch.long),
        "region_mask": torch.tensor([[[1.0], [1.0], [1.0], [1.0]]]),
        "alpha_mask": torch.tensor([[[0.8], [0.4], [0.1], [0.0]]]),
    }

    selected = zimage_minimal_inference.select_phase2_query_positions(region_state)
...
def test_phase1_reverse_adaptation_uses_branch_local_freqs_for_branch_context(monkeypatch):
    transformer = create_tiny_zimage_model()
    device = torch.device("cpu")
    cap_mask = torch.tensor([[True, True, True]], device=device)
    cap_feats = torch.randn(1, 3, 12, device=device)
    ...
    captured = {"branch_freqs": None, "background_context_branch_freqs": None}
    original_create_region_local_freqs = zimage_minimal_inference.create_region_local_freqs_for_caption_length

    def fake_create_region_local_freqs(*args, **kwargs):
        freqs = original_create_region_local_freqs(*args, **kwargs)
        captured["branch_freqs"] = freqs.clone()
        return freqs
    ...
    def fake_contextual_forward(
        ...
        if context_states is None:
            return query_states
        context_list = context_states if isinstance(context_states, list) else [context_states]
        freq_list = context_freqs_cis if isinstance(context_freqs_cis, list) else [context_freqs_cis]
        if include_query_in_kv and len(context_list) == 2:
            captured["background_context_branch_freqs"] = freq_list[1].clone()
            return query_states + 1.0
        return query_states
    ...
    monkeypatch.setattr(
        ...
        fake_create_region_local_freqs,
    )
    for layer in transformer.layers:
        monkeypatch.setattr(layer, "contextual_forward", types.MethodType(fake_contextual_forward, layer))
    ...
    assert captured["branch_freqs"] is not None
    assert captured["background_context_branch_freqs"] is not None
    assert torch.allclose(captured["background_context_branch_freqs"], captured["branch_freqs"])
