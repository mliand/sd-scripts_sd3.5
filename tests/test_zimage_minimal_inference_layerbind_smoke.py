...
def test_generate_image_writes_signal_probe_json(monkeypatch, tmp_path):
    def fake_encode_prompt(tokenize_strategy, encoding_strategy, text_encoder, prompt, _unused, device, dtype):
        embeds = torch.ones(1, 4, 8, device=device, dtype=dtype)
        mask = torch.tensor([[True, True, True, True]], device=device)
        return embeds, mask
    ...
    def fake_run_layerbind_forward(
        ...
        if signal_probe_accumulator is not None and phase == "phase1":
            zimage_minimal_inference.record_layerbind_signal_probe(
                ...
                branch_tokens=torch.tensor([[[4.0], [4.0], [1.0], [1.0]]]),
                current_tokens=torch.tensor([[[0.0], [0.0], [0.5], [0.5]]]),
                binary_mask=torch.tensor([[[1.0], [1.0], [0.0], [0.0]]]),
                branch_tokens_no_text=torch.tensor([[[1.0], [1.0], [0.5], [0.5]]]),
            )
        return torch.zeros_like(latent_model_input)
    ...
    output_dir = tmp_path / "out_signal"
    signal_probe_path = tmp_path / "signal_probes.json"
    zimage_minimal_inference.generate_image(
        transformer=FakeLayerBindTransformer(),
        vae=FakeVAE(),
        text_encoder=object(),
        ...
        prompt_dict={
            ...
            "layerbind_signal_probe_path": str(signal_probe_path),
        },
        output_dir=str(output_dir),
        output_name="signal",
        steps=2,
    ...
    assert signal_probe_path.exists()
    summary = json.loads(signal_probe_path.read_text(encoding="utf-8"))
    assert summary["global"]["d_fg_over_bg"] > 1.0
    assert summary["global"]["s_bg_over_fg"] < 1.0
