# Training SD3.5 with Gated Attention
# Based on sd3_train.py with gated attention support
...
from library import sd3_models_gated
...
def train(args):
    ...
    if args.train_t5xxl:
        assert args.train_text_encoder, "when training T5XXL, text encoder (CLIP-L/G) must be trained"
        assert not args.cache_text_encoder_outputs, "when training T5XXL, t5xxl output must not be cached"
    ...
    if args.train_text_encoder:
        ...
        lr_t5xxl = args.learning_rate_te3 if args.learning_rate_te3 is not None else args.learning_rate
    ...
    if args.cache_text_encoder_outputs:
        ...
        text_encoder_caching_strategy = strategy_sd3.Sd3TextEncoderOutputsCachingStrategy(
            ...
            train_clip or args.use_t5xxl_cache_only,
        ...
        # cache sample prompt's embeddings
    ...
    # Load Gated MMDiT
    logger.info(f"Loading Gated MMDiT with gate_type={args.gate_type}")
    ...
    if args.enable_scaled_pos_embed:
        ...
        latent_sizes = [round(math.sqrt(res[0] * res[1])) // 8 for res in resolutions]
    ...
    if not train_mmdit:
        mmdit.to(accelerator.device, dtype=weight_dtype)
    ...
    accelerator.print(f"gate_type: {args.gate_type}")
    ...
    # start training
    accelerator.print("running training / training start")
    ...
    logger.info(f"mmdit device: {accelerator.unwrap_model(mmdit).device}, dtype: {accelerator.unwrap_model(mmdit).dtype}" if mmdit else "mmdit is None")
    logger.info(f"clip_l device: {accelerator.unwrap_model(clip_l).device}, dtype: {accelerator.unwrap_model(clip_l).dtype}" if clip_l else "clip_l is None")
    logger.info(f"clip_g device: {accelerator.unwrap_model(clip_g).device}, dtype: {accelerator.unwrap_model(clip_g).dtype}" if clip_g else "clip_g is None")
    logger.info(f"t5xxl device: {accelerator.unwrap_model(t5xxl).device}, dtype: {accelerator.unwrap_model(t5xxl).dtype}" if t5xxl else "t5xxl is None")
    logger.info(f"vae device: {accelerator.unwrap_model(vae).device}, dtype: {accelerator.unwrap_model(vae).dtype}" if vae is not None else "vae is None")
    ...
    for epoch in range(num_train_epochs):
                ...
                if args.save_every_n_steps is not None and global_step % args.save_every_n_steps == 0:
                    ...
                    if accelerator.is_main_process:
                        sd3_train_utils.save_sd3_model_on_epoch_end_or_stepwise(
                            args, False, accelerator, save_dtype, epoch, num_train_epochs, global_step,
            ...
            if len(accelerator.trackers) > 0:
                ...
                if should_log_gate_stats:
                    ...
                    if hasattr(unwrapped_mmdit, 'get_gate_statistics'):
                        ...
                        if "gate_sparsity_overall" in gate_stats:
                            logs["gate/sparsity_overall"] = gate_stats["gate_sparsity_overall"]

                        # Optionally log per-block statistics
                        if args.log_gate_stats_detailed:
        ...
        if args.save_every_n_epochs is not None:
            if accelerator.is_main_process:
                sd3_train_utils.save_sd3_model_on_epoch_end_or_stepwise(
                    args, True, accelerator, save_dtype, epoch, num_train_epochs, global_step,
                    accelerator.unwrap_model(clip_l) if train_clip else None,
                    accelerator.unwrap_model(clip_g) if train_clip else None,
                    accelerator.unwrap_model(t5xxl) if train_t5xxl else None,
                    accelerator.unwrap_model(mmdit) if train_mmdit else None,
    ...
    if is_main_process:
        sd3_train_utils.save_sd3_model_on_train_end(
            args, save_dtype, epoch, global_step,
...
def setup_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--learning_rate_te1", type=float, default=None,
        help="learning rate for text encoder 1 (ViT-L)",
    )
    parser.add_argument(
        "--learning_rate_te2", type=float, default=None,
        help="learning rate for text encoder 2 (BiG-G)",
    )
    parser.add_argument(
        "--learning_rate_te3", type=float, default=None,
        help="learning rate for text encoder 3 (T5-XXL)",
    )

    parser.add_argument(
        "--skip_latents_validity_check", action="store_true",
        help="[Deprecated] use 'skip_cache_check' instead",
    )
    parser.add_argument(
        "--num_last_block_to_freeze", type=int, default=None,
    ...
    parser.add_argument(
        ...
        help="Type of gating for attention: 'headwise' (per-head scalar gate), 'elementwise' (per-element gate), or 'none' (no gating). Default: headwise",
    )
    parser.add_argument(
        "--log_gate_stats", action="store_true",
        help="Log gate statistics to tensorboard",
    )
    parser.add_argument(
        "--log_gate_stats_interval", type=int, default=100,
        help="Interval (in steps) for logging gate statistics. Default: 100",
    )
    parser.add_argument(
